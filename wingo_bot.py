...
START_HOUR  = 16   # e.g. 9   → সকাল ৯:০০ AM
START_MIN   = 0 # e.g. 0
STOP_HOUR   = 17  # e.g. 23  → রাত ১১:০০ PM
STOP_MIN    = 0  # e.g. 0
# None = 24/7 চলবে
...
def is_active_time():
    ...
    if START_HOUR is None or STOP_HOUR is None:
        return True  # No time restriction — 24/7
    now   = datetime.now()
    start = now.replace(hour=START_HOUR, minute=START_MIN or 0, second=0, microsecond=0)
    stop  = now.replace(hour=STOP_HOUR,  minute=STOP_MIN  or 0, second=0, microsecond=0)
    if start <= stop:
